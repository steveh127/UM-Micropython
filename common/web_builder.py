class Web_page():
	def __init__(self,*,title='',body='',css_file='',action_map=None):
		self.title=title
		self.body=body
		self.css=css_file
...
#create widget for text or integer input supplied value can either be a string or a method that returns a string
class Text_input(Widget):
	...
	def write(self):
		...
		#value is either a method returning a string or a string
		try:
			value=self.value()
		except:
			value=self.value
	...
	def save(self,item,values):
		if item == self.type_name:
			if self.integer:
				...
				self.value=clean_text(values[0])
				
#create widget for bulk text or input. Supplied value can either be a string or a method that returns a string			
class Text_area(Widget):
	def __init__(self,*,label='',command='text_area',cols=39,rows='6',value='',button_label='Submit',ID=''):
		self.label=label
		self.type_name=command
		self.cols=cols
		self.rows=rows
	...
	def write(self):
		text = '<div class="text_area"'
		if self.id:
			text += 'id="' + self.id + '" '
		text +=  '>\n\t<label>' + self.label + '</label>\n'
		text += '\t<textarea name="' + self.type_name + '" cols="' + str(self.cols) + '" rows="' + str(self.rows)
		try:
			value=self.value()
		except:
			value=self.value
		text += '" >' + value + '</textarea><br />\n\t'
		text +=	submit_button(self.type_name,self.button_label)
		text += '</div>\n'
		return text
...
class Checkbox(Widget):
	#just save state of properties
	def __init__(self,choices,*,label='Select some',command='check',value='no selections',button_label='Submit',ID='',button=True):
		self.choices=choices
...
#utility routine to process input text 
def clean_text(text):
	text=text.replace('+',' ')
	if '%' in text:
		...
		text=text.replace('%0D%0A',' ')	#crlf replaced with space
		#unrecognised '%' characters replaced with .
		while '%' in text:
