...
class ClockTimer():
	...
	async def finish(self):
		while True:
			if self.state == 'finished':
				if self.state == 'finished':
					await self.display.show('{RED}ALARM')
					await self.beep()
				await asyncio.sleep(1)
				if self.state == 'finished':
		...
		while True:
			...
			if b_start.value():
				...
				if self.state == 'alarming':
					...
					self.rgb(self.display.BLUE())
					self.state = 'clock_on'
					print('running alarm now')
					print(self.clock.show)
					await asyncio.sleep(.3)
				if self.state == 'finished':
					self.state = 'clock_on'
					await asyncio.sleep(.3)				
	...
	def alarm_action(self):
		self.alarm_hr,self.alarm_min = 0,0
		self.state='finished'
		self.alarm=False
	...
	def run_alarm(self):
		...
		if self.clock.ds:
			...
			if self.alarm_hr == -1:
				self.alarm_hr = 23
		self.clock.set_alarm((self.alarm_hr,self.alarm_min),self.alarm_action)	
		self.alarm=True
			...
			if self.state == 'alarming':
				...
				while self.state == 'alarming':
					if b_hour.value():
						...
						if self.mins == 24:
							self.mins=0
						self.alarm_hr = self.mins
						await asyncio.sleep(.3)
					if b_min.value():
						self.secs +=1
						if self.secs == 60:
							self.secs=0
						self.alarm_min = self.secs
						await asyncio.sleep(.3)
					await asyncio.sleep(0)
			await asyncio.sleep(1)
	...
	async def beep(self,freq=400,time=0.3,count=1,volume=200):
	...
	async def setup_tasks(self,b_set,b_start,b_hour,b_min):
		...
		asyncio.create_task(self.setup_alarm(b_hour,b_min))
