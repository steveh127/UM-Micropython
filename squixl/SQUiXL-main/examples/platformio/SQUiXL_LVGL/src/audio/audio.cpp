...
void AudioClass::play_tone(float _hz, float _tm)
{
	...
	vol = ((float)settings.config.volume / max_volume) / 6.0;
	if (!wav->isRunning())
	{
...
void AudioClass::play_dock()
{
	...
	// Playing this loud
	vol = ((float)settings.config.volume / max_volume) / 3.0;
	if (!wav->isRunning())
	{
		func = new AudioFileSourceFunction(20);
		func->addAudioGenerators([this](const float time) { return sine_wave(time); });
...
void AudioClass::play_menu_beep(int index)
{
	if (settings.config.volume == 0)
		return;
