...
bool RV3028C7::didCurrentTimeChange()
{
	// if (getCurrentDateTimeComponent(DATETIME_YEAR) < 23)
	// 	return false;
	...
	// Serial.printf("_dateTimeCached[sec] %d -> _dateTime[sec] %d -> chnaged? %s\n", _dateTimeCached[0], _dateTime[0], (_dateTimeCached[0] != _dateTime[0] ? "Y" : "N"));

	if (_dateTimeCached[0] != _dateTime[0] || _dateTimeCached[1] != _dateTime[1] || _dateTimeCached[2] != _dateTime[2])
	{

		_dateTimeCached[0] = _dateTime[0];
		_dateTimeCached[1] = _dateTime[1];
		_dateTimeCached[2] = _dateTime[2];
		_dateTimeCached[3] = _dateTime[3];
		_dateTimeCached[4] = _dateTime[4];
		_dateTimeCached[5] = _dateTime[5];
		_dateTimeCached[6] = _dateTime[6];
		changed = true;
	}
...
void RV3028C7::getCurrentTimeComponents(uint8_t *seconds, uint8_t *mins, uint8_t *hours)
{
	// Updates RTC date time value to array
	readBytesFromRegisters(REG_CLOCK_SECONDS, _dateTime, DATETIME_COMPONENTS);

	*seconds = convertToDecimal(_dateTime[DATETIME_SECOND]);
	*mins = convertToDecimal(_dateTime[DATETIME_MINUTE]);
	*hours = convertToDecimal(_dateTime[DATETIME_HOUR]);
}

void RV3028C7::getCurrentDateComponents(uint8_t *day_week, uint8_t *day_month, uint8_t *month, uint8_t *year)
{
	// Updates RTC date time value to array
	readBytesFromRegisters(REG_CLOCK_SECONDS, _dateTime, DATETIME_COMPONENTS);

	*day_week = convertToDecimal(_dateTime[DATETIME_DAY_OF_WEEK]);
	*day_month = convertToDecimal(_dateTime[DATETIME_DAY_OF_MONTH]);
	*month = convertToDecimal(_dateTime[DATETIME_MONTH]);
	*year = convertToDecimal(_dateTime[DATETIME_YEAR]);
}
